...
export P_MVAPICH=/picnic/scratch/ssgadmin/mvapich/2-2.0/
...
cat <<-EOF
	...
	export LD_LIBARY_PATH=$P_MVAPICH/lib/:$P_INTELCC/compiler/lib/intel64/:$LD_LIBARY_PATH
	
	mpirun_rsh -ssh -np \$LSB_DJOB_NUMPROC -hostfile \$LSB_DJOB_HOSTFILE $P_HPCG/xhpcg
EOF
